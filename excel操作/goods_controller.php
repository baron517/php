...
function importExecl($file=''){
	...
	for($j=0;$j<$sheetCount;$j++)
	{
    		...
    		for($_column=0; $_column<=$columnCnt; $_column++){
             ...
             if($cellValue)
             {
                 $data[$j][$_row][$cellName[$_column]] = $cellValue;
             }
    ...
    public function action_daoru()
    {
        
        
        echo "<meta http-equiv='Content-Type' content='text/html; charset=utf-8'><div style='text-align:center;margin-top:100px;font-size:25px;'>正在导入......</div>";
        
        $fileName="";
        
        //$fileName="doc/quanma.xlsx";
        ...
        {  
            ...
            if ($_FILES['file']['error'] > 0) {  
                ...
                $fileName=$DST_DIR.".".get_extension($_FILES['file']['name']);
                
                $upload=move_uploaded_file($_FILES['file']['tmp_name'], $fileName); 
       ...
       /* 
        ...
        for($i=1;$i<=sizeof($data[0]);$i++)
        {
            ...
            if($sheet>0)
            {
                ...
                {
                    $insertData["qm_money"]=$data[0][$i]['A'];
                    $insertData["qm_tiaojian"]=$data[0][$i]['D'];
                    $insertData["qm_youxiaoqi"]=$data[0][$i]['E'];
                    $insertData["qm_jifen"]=$data[0][$i]['F'];
                }
                
                
                for($j=1;$j<=sizeof($data[$sheet]);$j++)
                {
                    ...
                    if($j>1)
                    {
                        ...
                        $detailData["qm_code"]=$data[$sheet][$j]['B'];
                        
                        //$detailData["qm_code"]=time();
                                
                        //echo json_encode($detailData);
                        
                        
                        $quanma = new Model('quanma');
                        $findRs=$quanma->find(array("qm_code"=>$detailData["qm_code"]));
                
                        if(!$findRs)
                        {
                            $quanma->create($detailData); 
                        }
        ...
        header("Location:/index.php?m=backend&c=goods&a=quanma");
    ...
    {  
          ...
          if(request('step') == 'search')
        {
            ...
            if($kw != '')
            {
                $where .= ' AND qm_code LIKE :kw';
                $binds[':kw'] = '%'.$kw.'%';
            }
            
            
            $quanma = new Model("quanma");
            $total = $quanma->query("SELECT COUNT(*) as count FROM {$quanma->table_name} {$where}", $binds);
            
            
            if($total[0]['count'] > 0)
            {
                $fields = 'qm_id, qm_money, qm_tiaojian, qm_youxiaoqi, qm_jifen, qm_code,qm_status';
                $limit = $quanma->set_limit(array(request('page', 1), request('pernum', 10)), $total[0]['count']);
                
                $sort_id = request('sort_id', 0, 'post');
                $sort_map = array('qm_id DESC');
                $sort = isset($sort_map[$sort_id])? $sort_map[$sort_id] : $sort_map[0];
                
                $sql = "SELECT {$fields} FROM {$quanma->table_name} {$where} ORDER BY {$sort} {$limit}";
                       
                $results = array
                (
                    'status' => 'success',
                    'list' => $quanma->query($sql, $binds),
                    'paging' => $quanma->page,
                );
            }
            else
            ...
            $this->compiler('goods/quanma.html');
    ...
    public function action_add()
    {
        ...
        {   
            ...
            (
                ...
                'is_quanma' => (int)request('is_quanma', 0),
                'link' => trim(request('link', '')),
                'quanma_price' => (int)(request('quanma_price', '')),
